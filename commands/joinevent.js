...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName("eventid").setDescription("ID của sự kiện").setRequired(true)
  );
  ...
  try {
    ...
    if (!events.has(eventId)) {
      return interaction.reply({
        content: `Không tìm thấy sự kiện với ID: ${eventId}`,
        ephemeral: true,
      });
    }
    ...
    if (event.participants.includes(interaction.user.id)) {
      return interaction.reply({
        content: `Bạn đã tham gia sự kiện "${event.name}" rồi!`,
        ephemeral: true,
      });
    }
    ...
    await interaction.reply({
      content: `Bạn đã tham gia sự kiện "${event.name}" thành công!`,
      ephemeral: false,
    });
    ...
    await interaction.reply({
      content: "Có lỗi xảy ra khi tham gia sự kiện. Vui lòng thử lại sau.",
      ephemeral: true,
    });
  }
}
