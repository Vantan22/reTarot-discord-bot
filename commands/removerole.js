...
export const data = new SlashCommandBuilder()
  ...
  .addRoleOption((option) =>
    option.setName("role").setDescription("Vai trò cần xóa").setRequired(true)
  )
...
export async function execute(interaction) {
  ...
  try {
    ...
    if (!member.roles.cache.has(role.id)) {
      return interaction.reply({
        content: `${targetUser.tag} không có vai trò ${role.name}!`,
        ephemeral: true,
      });
    }
    ...
    await interaction.reply({
      content: `Đã xóa vai trò ${role.name} của ${targetUser.tag}!`,
      ephemeral: false,
    });
    ...
    await interaction.reply({
      content: "Có lỗi xảy ra khi xóa vai trò. Vui lòng thử lại sau.",
      ephemeral: true,
    });
  }
}
