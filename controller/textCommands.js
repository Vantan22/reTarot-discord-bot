...
export async function handleAddRole(author, channel) {
  try {
    ...
    const mentionedUsers = Array.from(
      channel.messages.cache.last().mentions.users.values()
    );
    const mentionedRoles = Array.from(
      channel.messages.cache.last().mentions.roles.values()
    );

    if (mentionedUsers.length === 0 || mentionedRoles.length === 0) {
      return channel.send(
        "Vui lòng đề cập đến người dùng và vai trò! Ví dụ: !addrole @user @role"
      );
    }
...
export async function handleEditRole(author, channel) {
  try {
    ...
    if (mentionedUsers.length === 0 || mentionedRoles.length < 2) {
      return channel.send(
        "Vui lòng đề cập đến người dùng, vai trò cũ và vai trò mới! Ví dụ: !editrole @user @oldrole @newrole"
      );
    }
    ...
    // Kiểm tra vị trí vai trò
    if (
      oldRole.position >= guild.members.me.roles.highest.position ||
      newRole.position >= guild.members.me.roles.highest.position
    ) {
      return channel.send(
        "Bot không thể quản lý vai trò cao hơn hoặc bằng vai trò cao nhất của bot!"
      );
    }
    ...
    channel.send(
      `Đã thay đổi vai trò từ ${oldRole.name} sang ${newRole.name} cho ${targetUser.username}!`
    );
  } catch (error) {
...
export async function handleRemoveRole(author, channel) {
  try {
    ...
    if (mentionedUsers.length === 0 || mentionedRoles.length === 0) {
      return channel.send(
        "Vui lòng đề cập đến người dùng và vai trò! Ví dụ: !removerole @user @role"
      );
    }
    ...
    if (role.position >= guild.members.me.roles.highest.position) {
      return channel.send(
        "Bot không thể xóa vai trò cao hơn hoặc bằng vai trò cao nhất của bot!"
      );
    }
...
export async function handleCreateEvent(author, args, channel) {
  try {
    ...
    if (parts.length < 3) {
      return channel.send(
        'Vui lòng cung cấp đủ thông tin! Ví dụ: !createevent Tên "Mô tả" "YYYY-MM-DD HH:MM"'
      );
    }
    ...
    if (
      ...
      return channel.send(
        'Vui lòng sử dụng dấu ngoặc kép cho mô tả và thời gian! Ví dụ: !createevent Tên "Mô tả" "YYYY-MM-DD HH:MM"'
      );
    }
    ...
    if (reminderDate > now) {
      ...
      setTimeout(() => {
        ...
        if (event) {
          const participantMentions = event.participants
            .map((id) => `<@${id}>`)
            .join(" ");
          channel.send(
            `${participantMentions} Nhắc nhở: Sự kiện "${eventName}" sẽ diễn ra sau 1 giờ nữa!`
          );
        }
      }, timeUntilReminder);
    }
