...
export async function execute(interaction) {
  ...
  try {
    ...
    if (!dateRegex.test(eventDateTime)) {
      return interaction.reply({ content: 'Định dạng thời gian không hợp lệ. Vui lòng sử dụng định dạng YYYY-MM-DD HH:MM.', ephemeral: true });
    }
      ...
      if (reminderTime > 0) {
        setTimeout(() => {
          ...
          if (event) {
            ...
            // Gửi nhắc nhở
            interaction.channel.send(`Nhắc nhở: Sự kiện "${event.name}" sẽ diễn ra sau 1 giờ nữa!\nNgười tham gia: ${participantsList}`);
          }
        }, reminderTime);
      }
    }
    ...
    await interaction.reply({ content: "Có lỗi xảy ra khi tạo sự kiện. Vui lòng thử lại sau.", ephemeral: true });
  }
}
